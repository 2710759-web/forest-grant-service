...
# ==========================================
# 🌟 ИНТЕРАКТИВНАЯ КАРТА С НАЛОЖЕНИЕМ ТОЧЕК НА ИЗОБРАЖЕНИЕ 🌟
# ==========================================
...
with col1:
    ...
    if not filtered_df.empty:
        ...
        # 2. Добавляем изображение КАК ФОН ГРАФИКА (не отдельно!)
        fig.add_layout_image(
            dict(
                ...
                yref="y",
                x=0,
                y=image.height,  # Верхний левый угол (Y инвертирован)
                sizex=image.width,
                sizey=image.height,
                sizing="stretch",
                opacity=1.0,
                layer="below"  # Картинка ПОД точками
            )
        )
        ...
        # 6. Настройка осей (ВАЖНО: range должен соответствовать размерам изображения)
        fig.update_layout(
            xaxis=dict(
                range=[0, image.width],  # От 0 до ширины изображения
                showgrid=False,
                zeroline=False,
                visible=False  # Скрываем оси
            ),
            yaxis=dict(
                range=[image.height, 0],  # Инвертируем Y (0 сверху, height снизу)
                showgrid=False,
                zeroline=False,
                visible=False
            ),
