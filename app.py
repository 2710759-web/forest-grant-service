...
def get_image_base64(img):
    """Конвертирует PIL Image в base64 строку для надежного отображения в Plotly"""
    buffered = io.BytesIO()
    # Сохраняем как PNG для гарантированной совместимости с Plotly
...
uploaded_image = st.sidebar.file_uploader("Загрузите ортофотоплан (JPG, PNG, TIF)", type=["jpg", "jpeg", "png", "tif", "tiff"])
...
if demo_mode:
    ...
    try:
        ...
        # Конвертируем в RGB, если это TIFF или RGBA, чтобы избежать ошибок
        if image.mode != 'RGB':
...
# Генерация ID для интерактивности
...
with col1:
    ...
    if not filtered_df.empty:
        ...
        # 1. Конвертируем изображение в base64 для надежного отображения
        img_base64 = get_image_base64(image)
        
        # 2. Добавляем фоновое изображение СТРОГО ПОД ТОЧКАМИ (layer="below")
        fig.add_layout_image(
            dict(
                source=img_base64,
                xref="x", yref="y",
                x=0, y=image.height,  # y=image.height, чтобы перевернуть ось Y как в картинке (0 сверху)
                sizex=image.width, sizey=image.height,
                sizing="stretch", 
                opacity=1.0, 
                layer="below" # Ключевой параметр: картинка будет под точками
            )
        )
        ...
        # 5. Добавляем интерактивный scatter plot
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                ...
                opacity=0.9 # Чуть повысил непрозрачность, чтобы точки лучше читались на фоне
            ),
            text=hover_texts,
            hoverinfo='text',
            name='Деревья'
        ...
        # 6. Настройка осей и макета (скрываем оси, но сохраняем масштаб изображения)
        fig.update_layout(
            xaxis=dict(range=[0, image.width], showgrid=False, zeroline=False, visible=False),
            yaxis=dict(range=[image.height, 0], showgrid=False, zeroline=False, visible=False),
