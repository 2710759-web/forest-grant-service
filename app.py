...
def calculate_additional_params(df, species=None):
    ...
    # Определяем, есть ли колонка "Порода" в данных
    ...
    # 2. Видовое число (f) - зависит от породы
...
# Проверяем, есть ли колонка "Порода"
has_species_column = 'Порода' in df.columns

# Выбор породы (только если в данных нет колонки "Порода")
species = None
if not has_species_column:
    ...
    st.sidebar.success("✅ В таблице обнаружена колонка 'Порода'. Расчёты выполняются для каждой породы индивидуально.")
    # Показываем распределение пород
...
with col2:
    ...
    if has_species_column:
        ...
        for sp, count in df['Порода'].value_counts().items():
            st.write(f"• {sp}: {count} дер.")
    else:
