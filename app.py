...
# --- ЗАГРУЗКА ДАННЫХ ---
# Путь к файлам (положите их в ту же папку, где лежит этот скрипт)
IMG_PATH = "forest.jpg"
XLSX_PATH = "trees.xlsx"

if not os.path.exists(IMG_PATH) or not os.path.exists(XLSX_PATH):
    st.error("Ошибка: Файлы 'forest.jpg' и 'trees.xlsx' не найдены в папке со скриптом.")
    st.stop()

# Загружаем изображение и таблицу
@st.cache_data
def load_data():
...
with col1:
    st.subheader("Карта выделенных территорий")
